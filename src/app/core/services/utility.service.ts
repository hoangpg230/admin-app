...
export class UtilityService {
  ...
  setData(localKey: string, values: any) {
    localStorage.setItem(localKey, JSON.stringify(values))
  }
  getData(localKey: string) {
    let data
    let TotalRecords
    let tmp = JSON.parse(localStorage.getItem(localKey) || '{}')
    localStorage.removeItem(localKey)
    data = tmp[`_list${localKey}`]
    TotalRecords = ~~tmp.totalRecords
    return {
      data, TotalRecords
    }
  }
